      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <StaggerContainer className="grid grid-cols-1 lg:grid-cols-2 gap-5" stagger={0.08}>
          {odooModuleCategories.map((cat) => (
            <StaggerItem key={cat.category} className="bg-blue-light rounded-2xl p-6 border border-line">
              <h3 className="font-bold text-navy mb-4">{cat.category}</h3>
              <div className="flex flex-wrap gap-2.5">
                {cat.modules.map((m, i) => (
                  <div key={m.label} title={m.label} className="flex flex-col items-center gap-1 w-16 text-center">
                    ...
                    <span className="text-[10px] leading-tight text-ink font-medium">{m.label}</span>
                  </div>
                ))}
              </div>
            </StaggerItem>
