...
export default function BiSectors() {
  const t = useTranslations("Bi.sectors");
              ...
              <StaggerItem key={l(s.sector, "fr")}>
                <div className="h-full bg-white border border-line rounded-2xl p-5 transition-[transform,border-color] hover:-translate-y-0.5 hover:border-orange">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h3 className="font-bold text-navy text-sm">{l(s.sector, locale)}</h3>
                  </div>
                  <p className="text-sm text-ink leading-relaxed mb-2">{l(s.useCases, locale)}</p>
                  <p className="text-xs font-semibold text-navy flex items-start gap-1.5">
                    <span className="text-orange" aria-hidden="true">▸</span> {l(s.benefit, locale)}
                  </p>
                </div>
              </StaggerItem>
            );
