...
import { tdResources } from "@/lib/transformation-digitale-data";
import { l } from "@/lib/i18n-content";

export default function TdResources() {
  const t = useTranslations("Td.resources");
        ...
        <StaggerContainer className="grid grid-cols-1 sm:grid-cols-2 gap-5" stagger={0.08}>
          {tdResources.map((r) => (
            ...
            >
              ...
              <h3 className="font-bold text-navy mb-2 pr-24">{l(r.title, locale)}</h3>
              <p className="text-sm text-ink">{l(r.desc, locale)}</p>
            </StaggerItem>
          ))}
        </StaggerContainer>
      </div>
